...
public class OrderTotalApprovePersistenceTest extends BasePersistenceTest
{
    @Before
    ...
    @Test
    public void testOrderTotalsApprove()
        throws Exception
    {
        executeInATransaction(new Callable<Object>() {
            public Object call() throws Exception {
                ...
                modelUtils.approve(approverUser, order, true, null) ;
                
                Assert.assertEquals("Buyer total", order.getTotal(), buyerUser.getBuyerTotal().getTotalBought()) ;
                Assert.assertEquals("Approved total", order.getTotal(), buyerUser.getApprovedTotal().getApproved()) ;
                Assert.assertEquals("Rejected total", 0, approverUser.getApproverTotal().getRejected()) ;
                Assert.assertEquals("Approver approved total", order.getTotal(), approverUser.getApproverTotal().getApproved()) ;
                Assert.assertEquals("Approver rejected total", 0, buyerUser.getApprovedTotal().getRejected()) ;
                
                Assert.assertNull("Rejection message", order.getRejectionMessage()) ;
                return null ;
            }
        }) ;
    }
